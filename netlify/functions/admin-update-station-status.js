...
const STATIONS = ['Bar', 'Kitchen', 'Wait Station', 'Runner'];
...
exports.handler = async (event) => {
  try {
    ...
    if (!body.orderId || !STATIONS.includes(body.station) || !STATUSES.includes(body.status)) {
      return json(400, { ok: false, error: 'Invalid station status update.' });
    }
    ...
    return json(500, { ok: false, error: err.message || 'Unable to update station.' });
  }
};
