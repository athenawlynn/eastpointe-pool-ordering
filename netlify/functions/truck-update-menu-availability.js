...
exports.handler = async (event) => {
  try {
    ...
    if (!body.itemId || typeof body.available !== 'boolean') {
      return json(400, { ok: false, error: 'Invalid truck menu availability update.' });
    }
    ...
    const response = await fetch(scriptUrl, {
      ...
      body: JSON.stringify({
        action: 'updateTruckMenuAvailability',
        adminKey,
        itemId: body.itemId,
    ...
    return json(500, { ok: false, error: err.message || 'Unable to update truck menu availability.' });
  }
};
